...
public class DateUtils {
  ...
  public static Boolean between(LocalDate now, LocalDate startTime, LocalDate endTime) {
    return (now.isAfter(startTime) && now.isBefore(endTime)) || startTime.equals(now) || endTime
        .equals(now);
  }

  public static String outTime(LocalDateTime now, LocalDateTime target) {
    Duration duration = Duration.between(target, now);
    if (duration.toDays() > 0) {
      return StrUtil.format("{} 天之前", duration.toDays());
    } else if (duration.toHours() > 0) {
      return StrUtil.format("{} 小时之前", duration.toHours());
    } else {
      return StrUtil.format("{} 分钟之前", duration.toMinutes());
    }
  }
}
